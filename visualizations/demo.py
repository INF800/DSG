...
def get_parser():
    ...
    parser.add_argument(
        "--raw_output",
        help="A file or directory to save the raw output tensors. "
    )
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            predictions, visualized_output, visualized_output_relations = demo.run_on_image(img)
            logger.info(
                "{}: {} in {:.2f}s".format(
                    path,
            ...
            if args.output:
                ...
                if cv2.waitKey(0) == 27:
                    break  # esc to quit
            if args.raw_output:
                assert os.path.isdir(args.raw_output)
                out_filename_raw = os.path.join(args.raw_output, os.path.basename(path))
                out_filename_raw = out_filename_raw.replace('png','pt')
                torch.save(predictions, out_filename_raw)
